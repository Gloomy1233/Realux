...
const LEGACY_MAGIC = utf8ToBytes('\nREALUX_EMBED_V1:');
...
function isRealuxComComment(comment: Uint8Array): boolean {
  return new TextDecoder().decode(comment).startsWith(PROOF_COM_PREFIX);
}
...
function extractFromComSegments(raw: Uint8Array): EmbeddedProofEnvelope | null {
  let i = 0;
  ...
  while (i < raw.length) {
    ...
    if (marker === 0xfe) {
      ...
      const text = new TextDecoder().decode(comment);
...
export function embedProofEnvelope(raw: Uint8Array, envelope: EmbeddedProofEnvelope): Uint8Array {
  ...
  const payload = utf8ToBytes(`${PROOF_COM_PREFIX}${bytesToBase64Url(utf8ToBytes(JSON.stringify(envelope)))}`);
...
export async function appendProofEnvelope(imageUri: string, envelope: EmbeddedProofEnvelope): Promise<string> {
  const raw = stripKnownProofTrailers(await readImageBytes(imageUri));
  ...
  const outPath = `${cacheDir}realux-proof-${envelope.captureId}.jpg`;
