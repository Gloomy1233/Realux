...
import { base64ToBytes, base64UrlToBytes, bytesToBase64, bytesToBase64Url, utf8ToBytes } from '@/lib/authenticity/encoding';
import type { EmbeddedProofEnvelope } from '@/types/authenticity';

const PROOF_MAGIC = utf8ToBytes('\nREALUX_PROOF_V2:');
...
function parseProofPayload(encoded: string): EmbeddedProofEnvelope {
  const parsed = JSON.parse(new TextDecoder().decode(base64UrlToBytes(encoded.trim()))) as EmbeddedProofEnvelope;
  return parsed;
}

function extractFromUuidBox(data: Uint8Array, box: TopLevelBox): EmbeddedProofEnvelope | null {
  const payloadStart = box.headerEnd + 16;
  const payloadEnd = box.start + box.size;
  if (payloadStart >= payloadEnd) return null;
  const text = new TextDecoder().decode(data.slice(payloadStart, payloadEnd));
...
function buildUuidProofBox(payloadText: string): Uint8Array {
  const payload = utf8ToBytes(payloadText);
...
export function embedProofEnvelope(raw: Uint8Array, envelope: EmbeddedProofEnvelope): Uint8Array {
  const canonical = stripKnownProofTrailers(raw);
  if (!isMp4Container(canonical)) {
    throw new Error('Invalid MP4: ftyp box not found.');
  }

  const payloadText = `${PROOF_COM_PREFIX}${bytesToBase64Url(utf8ToBytes(JSON.stringify(envelope)))}`;
...
export async function readVideoBytes(uri: string): Promise<Uint8Array> {
  const b64 = await FileSystem.readAsStringAsync(uri, { encoding: FileSystem.EncodingType.Base64 });
  return base64ToBytes(b64);
}

export async function appendProofEnvelope(videoUri: string, envelope: EmbeddedProofEnvelope): Promise<string> {
  const raw = stripKnownProofTrailers(await readVideoBytes(videoUri));
  const out = embedProofEnvelope(raw, envelope);

  const cacheDir = FileSystem.cacheDirectory;
  if (!cacheDir) {
    throw new Error('Cache directory unavailable');
  }
  const outPath = `${cacheDir}realux-proof-${envelope.captureId}.mp4`;
  await FileSystem.writeAsStringAsync(outPath, bytesToBase64(out), {
    encoding: FileSystem.EncodingType.Base64,
  });
  return outPath;
}

export function extractProofEnvelope(raw: Uint8Array): EmbeddedProofEnvelope | null {
  const trailerIdx = findBytes(raw, PROOF_MAGIC);
  if (trailerIdx >= 0) {
    const encoded = new TextDecoder().decode(raw.slice(trailerIdx + PROOF_MAGIC.length)).trim();
