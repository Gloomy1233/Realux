...
function stripTrailingProof(raw) {
...
function isRealuxComComment(comment) {
    return bytesToUtf8(comment).startsWith(PROOF_COM_PREFIX);
}
function findSosStartIndex(raw) {
    ...
    while (i < raw.length) {
        ...
        const markerStart = i;
        let markerPos = i + 1;
        while (markerPos < raw.length && raw[markerPos] === 0xff)
            markerPos++;
...
function buildComSegment(comment) {
...
function stripComProofSegments(raw) {
    const chunks = [];
    let i = 0;
    if (raw.length >= 2 && raw[0] === 0xff && raw[1] === 0xd8) {
    ...
    while (i < raw.length) {
        ...
        if (marker === 0xda) {
            ...
            if (i < raw.length)
                chunks.push(raw.slice(scanStart));
            break;
        }
        if (isStandaloneMarker(marker)) {
            chunks.push(raw.slice(markerStart, markerPos + 1));
...
export function stripKnownProofTrailers(raw) {
    return stripComProofSegments(stripTrailingProof(raw));
}
export function embedProofEnvelope(raw, envelope) {
    ...
    const payload = new TextEncoder().encode(`${PROOF_COM_PREFIX}${bytesToBase64Url(new TextEncoder().encode(JSON.stringify(envelope)))}`);
...
export function extractProofEnvelope(raw) {
