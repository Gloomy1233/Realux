...
function uuidMatches(data, offset) {
    for (let i = 0; i < REALUX_MP4_PROOF_UUID.length; i++) {
        if (data[offset + i] !== REALUX_MP4_PROOF_UUID[i])
            return false;
    }
    return true;
}
function parseTopLevelBoxes(data) {
    const boxes = [];
    let offset = 0;
...
function isRealuxUuidBox(data, box) {
    if (box.type !== 'uuid')
        return false;
    if (box.headerEnd + 16 > box.start + box.size)
        return false;
...
function stripUuidProofBoxes(raw) {
    const boxes = parseTopLevelBoxes(raw);
    if (!boxes.length)
        return raw;
    const chunks = [];
    for (const box of boxes) {
        if (isRealuxUuidBox(raw, box))
            continue;
...
function extractFromUuidBox(data, box) {
...
function buildUuidProofBox(payloadText) {
...
export function isMp4Container(raw) {
    const boxes = parseTopLevelBoxes(raw);
