...
import { embedProofEnvelope, extractProofEnvelope, isMp4Container, stripKnownProofTrailers, } from './mp4Proof.js';
...
test('uuid proof is inserted before mdat', () => {
    ...
    while (offset + 8 <= embedded.length) {
        const size = ((embedded[offset] << 24) |
            (embedded[offset + 1] << 16) |
            (embedded[offset + 2] << 8) |
            embedded[offset + 3]) >>>
            0;
        const type = String.fromCharCode(embedded[offset + 4], embedded[offset + 5], embedded[offset + 6], embedded[offset + 7]);
        order.push(type);
        if (size < 8)
            break;
        offset += size;
