...
import { registerVideoCaptureWithBackend } from '@/lib/authenticity/registerVideoCapture';
...
export default function ProcessVideoScreen() {
  ...
  const { uri, w, h, durationMs } = useLocalSearchParams<{
    uri: string;
    w: string;
    h: string;
    durationMs: string;
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      try {
        ...
        setStep('embed');
        setStep('upload');
        const res = await registerVideoCaptureWithBackend({
          localUri: uri,
          uid: user.uid,
          deviceId,
          width: Number(w) || 1,
        ...
        if (e instanceof FirebaseError && e.code === 'functions/unauthenticated') {
          ...
          message = emu
            ...
            : `The callable saw no logged-in Firebase user (${projectId}). Sign out and sign in again after a full app restart (not just Reload).`;
        }
        setErr(message);
        setStep('error');
      }
  ...
  }, [uri, w, h, durationMs, user?.uid, deviceId, deviceSecret]);
  ...
  if (step === 'done' && mediaId) {
    return (
      <View style={styles.center}>
        <Text style={styles.title}>Video registration complete</Text>
        <Text style={styles.body} selectable>
          captureId: {mediaId}
        </Text>
        <Text style={styles.hint}>
          Verify this capture from Verify video → Saved videos on this device for the most reliable result.
        </Text>
        {certificateId ? (
          <Text style={styles.body} selectable>
  ...
  return (
    <View style={styles.center}>
      ...
      <Text style={styles.body}>Working through video registration steps…</Text>
      <Text style={styles.step}>
        {step === 'hash' && 'Hashing video'}
